...
int main(void)
{
	...
	{	
		...
		else if(!authentication)
		{
			...
			debug_printf ("buf : %s code : %s\n", buf, code);
			if(!strcmp(code, buf))
			{
				if((numbytes = sendto(sockfd, code, strlen(code), 0, (struct sockaddr *)&their_addr, sizeof their_addr)) == -1) 
				{
					perror("sendto");
					exit(1);
				}
				...
				{	
					debug_printf("Autheticated\n");
					debug_printf("code sent %s\n",code);
					authentication = 1;
				}	
			}
			else 
