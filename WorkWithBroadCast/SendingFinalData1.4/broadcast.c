...
/* Program to create udp socket with threading */
...
int main(int argc, char *argv[])
{
	...
	while(1) 
	{
		...
		if (FD_ISSET(sockfd, &readfds)) 
		{
			...
			IPIndex ++;
			IPAddressCounter++;
			debug_printf("IPAddressCounter = %d, IPAddress[%d] %s\n",IPAddressCounter,  IPIndex-1, IPAddress[IPIndex-1]);
		}
		else
		{
	...
	for(i = 0; i<IPIndex; i++)
		debug_printf("IPAddress[%d] : %s\n", i, IPAddress[i]);
	debug_printf("\n");
	
	createPosixThreadRetrun = createPosixThread(IPAddress, &IPIndex);
